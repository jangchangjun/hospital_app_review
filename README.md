...
이 프로젝트에서는 앱 리뷰 데이터를 통해 긍,부정을 판별하는 인공지능 모델을 만들고, 학습을 통해 판별된 데이터로 사용자의 리뷰를 분석해 이를 기반으로 서비스 개선 방안을 제시할 수 있는 프로그랭을 만들고자한다.
...
이 단계에서 활용할 데이터는 두 개의 모델중 정확도가 더 높았던 2번째 모델이 예측한 데이터셋을 활용할 것이다.
...
2019, 2015, 2020년의 리뷰는 긍정이 많은 반면 2017,2023, 2024년의 리뷰는 부정 비율이 많은 것을 알 수 있다.
...
그래프에 보이다시피 코로나로 인해 2019년도의 리뷰수가 급증했으며, 그 뒤로 2020, 2022, 2023년이 뒤따라 오는것을 확인 할 수 있다.

<div><p align='center'><img src="img/년별긍부정.png" width = "500", height = "400"></p></div>

연별 리뷰의 긍부정 분포 그래프이다.

2019, 2015, 2020년의 리뷰는 긍정이 많은 반면 2017,2023, 2024년의 리뷰는 부정 비율이 높은 것을 확인 할 수 있다.
...
2017년의 리뷰 수는 1월에 집중되어있으며, 긍부정 비율은 1-3월과 7월 이후에 긍정과 부정의 비율이 비슷하거나 부정의 비율이 더 많아진 것을 알 수 있다.
...
이는 코로나 펜데믹 시절 똑닥의 서비스에 사용자 대부분의 만족도가 높았음을 말해준다.
...
위 그래프중 부정 비율이 높았던 2017년과 2024년도에 사용자 이탈률을 리뷰수의 증감과 긍부정 추이를 토대로 분석해보고자 한다.

-토픽모델링 사용하기 으이이이이이이 ㅗㅗ
